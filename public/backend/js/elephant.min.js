...
function(a) { "use strict";
    ...
    function b() { var a = document.createElement("bootstrap"),
            b = { WebkitAnimation: "webkitAnimationEnd", MozAnimation: "mozAnimationEnd", msAnimation: "MSAnimationEnd", OAnimation: "oanimationend", animation: "animationend" }; for (var c in b)
            if (void 0 !== a.style[c]) return { end: b[c] };
        return !1 }
    a.fn.emulateAnimationEnd = function(b) { var c = this,
            d = !1;
        a(this).one("bsAnimationEnd", function(a) { d = !0 }); var e = function() { d || a(c).trigger(a.support.animation.end) }; return setTimeout(e, b), this }, a(function() { a.support.animation = b(), a.support.animation && (a.event.special.bsAnimationEnd = { bindType: a.support.animation.end, delegateType: a.support.animation.end, handle: function(b) { if (a(b.target).is(this)) return b.handleObj.handler.apply(this, arguments) } }) }) }(jQuery),
function(a) { "use strict";

    function b(b) { return this.each(function() { var d = a(this),
                e = d.data("bs.input");
            e || d.data("bs.input", e = new c(this)), "string" == typeof b && e[b]() }) } var c = function(b) { this.$element = a(b), this.$wrapper = null, this.init() };
    c.VERSION = "1.0.0", c.prototype.checkValue = function() { var a = (this.$element.val() || "").length;
        this.$wrapper.toggleClass("has-value", !!a) }, c.prototype.checkDisabled = function() { var a = this.$element.is(":disabled");
        this.$wrapper.toggleClass("is-disabled", !!a) }, c.prototype.updateCssClasses = function() { this.checkValue(), this.checkDisabled() }, c.prototype._handleInputFocus = function() { this.$wrapper.addClass("is-focused") }, c.prototype._handleInputBlur = function() { this.$wrapper.removeClass("is-focused") }, c.prototype.init = function() { this.$wrapper = this.$element.closest(".md-form-group"), this.$element.on("change.bs.input", a.proxy(this.updateCssClasses, this)).on("focus.bs.input", a.proxy(this._handleInputFocus, this)).on("blur.bs.input", a.proxy(this._handleInputBlur, this)), this.updateCssClasses() }; var d = a.fn.input;
    a.fn.input = b, a.fn.input.Constructor = c, a.fn.input.noConflict = function() { return a.fn.input = d, this }, a(".md-form-control").each(function() { b.call(a(this)) }) }(jQuery),
function(a) { "use strict";

    function b(b, c) { var e = a(b),
            f = e.data("bs.chart"); if (f) return f; var g = {};
        g.type = c.type || c.chart, g.type = a.camelCase(g.type), d(g, "data.labels", c.labels), d(g, "data.datasets", a.map(c.values, function(b) { return ~a.inArray("points", c.hide) && (b.pointBackgroundColor = "transparent", b.pointBorderColor = "transparent"), b })), d(g, "options", c), d(g, "paths", { scalesX: "scales.xAxes[0].display", scalesY: "scales.yAxes[0].display", gridLinesX: "scales.xAxes[0].gridLines.display", gridLinesY: "scales.yAxes[0].gridLines.display", tooltips: "tooltips.enabled", legend: "legend.display" }), a.each(c.hide, function(a, b) { g.paths[b] && g.paths[b].length && d(g.options, g.paths[b], !1) }), f = new Chart(e, g), e.data("bs.chart", f) }

    function c(a, b) { return a.replace(/\[/g, ".").replace(/]/g, "").split(b) }

    function d(a, b, e) { var f = c(b, "."),
            g = f.shift(); if (0 === f.length) return a[g] = e; var h = "number" == typeof f[0];
        a[g] || (a[g] = h ? [] : {}), d(a[g], f.join("."), e) }
    a.fn.chart = function(a) { var c = this; return this.each(function() { var d = b(this, a); if ("instance" === a) return c = d, !1 }), c }, a("[data-chart]").each(function() { var b = a(this),
            c = b.data();
        b.chart(c) }) }(jQuery),
function(a) { "use strict";

    function b(b, d) { var e = a(b),
            f = e.data("bs.slider"); return f ? f : (d = a.extend({}, c, d), a.isNumeric(d.start) && (d.start = [d.start]), a.isEmptyObject(d.range) && (d.range = { max: d.max, min: d.min }), e.addClass([d.cssPrefix.replace(/-$/, ""), d.cssPrefix.concat(d.slider)].join(" ")), f = noUiSlider.create(b, d), void e.data("bs.slider", f)) } var c = { connect: "lower", cssPrefix: "slider-", max: 100, min: 0, range: {}, slider: "default", start: 0 };
    a.fn.slider = function(a) { var c = this; return this.each(function() { var d = b(this, a); if ("instance" === a) return c = d, !1 }), c }, a("[data-slider]").each(function() { var b = a(this),
            c = b.data(); if (b.slider(c), c.target && c.target.length) { var d = b.slider("instance"),
                e = a(c.target.toString());
            d.on("update", function(a, b) { e.eq(b)[e.eq(b).is("input") ? "val" : "text"](a[b]) }) } }) }(jQuery),
function(a) { a.validator.setDefaults({ errorElement: "small", errorClass: "has-error", errorIcon: "icon-times", validClass: "has-success", validIcon: "icon-check", errorPlacement: function(a, b) { var c = b.closest("form"),
                d = b.parents(".form-group").last();
            c.hasClass("form-horizontal") && (d = b.parent()), a.addClass("help-block").appendTo(d) }, highlight: function(b, c, d) { var e = a.extend({}, this.settings),
                f = a(b).closest(".form-group"); if (f.hasClass("has-feedback")) { var g = f.find(".form-control-feedback").children();
                g.addClass(e.errorIcon).removeClass(e.validIcon) }
            f.addClass(c).removeClass(d).find(".help-block").not(".has-error").hide() }, unhighlight: function(b, c, d) { var e = a.extend({}, this.settings),
                f = a(b).closest(".form-group"); if (f.hasClass("has-feedback")) { var g = f.find(".form-control-feedback").children();
                g.addClass(e.validIcon).removeClass(e.errorIcon) }
            f.addClass(d).removeClass(c).find(".help-block").not(".has-error").show() } }), a('[data-toggle="validator"]').each(function() { var b = a(this),
            c = b.data();
        b.validate(c) }) }(jQuery),
function(a) { a.validator.setDefaults({ errorElement: "span", errorClass: "has-error", validClass: "has-success", errorPlacement: function(a, b) { var c = b.parents(".md-form-group").last();
            a.addClass("md-help-block").appendTo(c) }, highlight: function(b, c, d) { a(b).closest(".md-form-group").addClass(c).removeClass(d).find(".md-help-block").not(".has-error").hide() }, unhighlight: function(b, c, d) { a(b).closest(".md-form-group").addClass(d).removeClass(c).find(".md-help-block").not(".has-error").show() } }), a('[data-toggle="md-validator"]').each(function() { var b = a(this),
            c = b.data();
        b.validate(c) }) }(jQuery);