        ...
        <div class="col-sm-12">
            <div class="card">
                ...
                <div class="row" style="margin:10px">
                    <form action="{{ route('admin.menus.update', $menu->id) }}" method="post" class="form-horizontal form-label-left">
                        @csrf
                        @method('put')
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <input type="text" name="menu"
                                    class="form-control @error('menu') is-invalid @enderror col-md-7 col-xs-12" value="{{$menu->menu}}">
                                @error('menu')
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select name="parent_id" class="form-control @error('parent_id') is-invalid @enderror">
                                  ...
                                  @foreach ($parents as $item)
                                       <option value="{{$item->id}}" @if($item->id == $menu->parent_id) selected @endif >{{$item->menu}}</option>
                                  @endforeach
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select name="page_structure" class="form-control @error('page_structure') is-invalid @enderror">
                                     @foreach ($structure as $item)
                                          <option value="{{$item}}" @if($menu->page_structure == $item) selected @endif>{{$item}} </option>
                                     @endforeach
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <input type="number" class="form-control @error('sequence') is-invalid @enderror"
                                    name="sequence" value="{{$menu->sequence}}" style="width:30%">
                                @error('sequence')
                                    <div class="py-3 " style="color:#f00">{{ $message }}</div>
                                @enderror
                            </div>
